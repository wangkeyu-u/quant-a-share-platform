"""次日方向分类与 horizon 期收益回归。

预测日之前已结束的标签才可进入训练。Holdout/CV 按日期分组并清除跨越
验证边界的标签。这个约束不证明复权数据、交易执行或策略选择没有其他偏差。
"""
from __future__ import annotations

import os

import joblib
import numpy as np
import pandas as pd
from sklearn.metrics import accuracy_score, mean_absolute_error, r2_score, roc_auc_score
from sklearn.model_selection import TimeSeriesSplit, cross_val_score

from quant.ai.gateway import get_gateway
from quant.ml.features import (
    CLS_TARGET, FEATURE_COLS, REG_TARGET, FEATURE_DATE, LABEL_END, build_features,
)

_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
MODELS_DIR = os.path.join(_ROOT, "models")

# 超参搜索网格(在训练集上做时序交叉验证选优)
PARAM_GRID = [
    {"n_estimators": 200, "max_depth": 3, "learning_rate": 0.05},
    {"n_estimators": 300, "max_depth": 4, "learning_rate": 0.03},
    {"n_estimators": 400, "max_depth": 5, "learning_rate": 0.02},
    {"n_estimators": 500, "max_depth": 6, "learning_rate": 0.02},
]


def _clean(feats: pd.DataFrame, dropna: bool = True) -> pd.DataFrame:
    if dropna:
        return feats.dropna(subset=FEATURE_COLS + [CLS_TARGET, REG_TARGET])
    return feats


def _known_before(feats: pd.DataFrame, cutoff) -> pd.DataFrame:
    """Both the observation and its complete target must precede cutoff."""
    return feats[(feats[FEATURE_DATE] < cutoff) & (feats[LABEL_END] < cutoff)]


def _holdout(feats: pd.DataFrame, test_size: float):
    if not 0 < test_size < 1:
        raise ValueError("test_size must be between zero and one")
    dates = np.sort(feats[FEATURE_DATE].unique())
    split = int(len(dates) * (1 - test_size))
    if split < 1 or split >= len(dates):
        raise ValueError("not enough distinct dates for holdout")
    cutoff = dates[split]
    train = _known_before(feats, cutoff)
    test = feats[feats[FEATURE_DATE] >= cutoff]
    if train.empty or test.empty:
        raise ValueError("not enough labeled history after purging")
    return train, test


def _purged_splits(feats: pd.DataFrame):
    """Integer row indices for sklearn; same-date symbols stay together."""
    dates = np.sort(feats[FEATURE_DATE].unique())
    splits = []
    for train_dates, validation_dates in TimeSeriesSplit(n_splits=3).split(dates):
        cutoff = dates[validation_dates[0]]
        train = (feats[FEATURE_DATE].isin(dates[train_dates]) & (feats[LABEL_END] < cutoff))
        validation = feats[FEATURE_DATE].isin(dates[validation_dates])
        if not train.any():
            raise ValueError("not enough labeled history for purged CV")
        splits.append((np.flatnonzero(train), np.flatnonzero(validation)))
    return splits


def _best_params(X, y, make_fn, scoring, cv):
    """时序交叉验证选最优超参(所有 estimator 构造都经网关)。"""
    best, best_s = None, -np.inf
    for p in PARAM_GRID:
        try:
            est = make_fn(p)
            s = cross_val_score(est, X, y, cv=cv, scoring=scoring, n_jobs=1)
            m = s.mean()
        except Exception:
            continue
        if m > best_s:
            best_s, best = m, p
    return best or PARAM_GRID[0], (float(best_s) if np.isfinite(best_s) else None)


def train_models(df: pd.DataFrame, symbol: str, horizon: int = 5,
                 model_path: str | None = None, test_size: float = 0.2) -> dict:
    """在全部历史上按时间切分训练/测试,训练双模型并保存,返回评估指标。

    切分方式:前 (1-test_size) 用于训练 + 超参搜索,后 test_size 作为样本外测试集,
    报告里的指标就是这段"没见过的数据"上的表现。训练与预测均经 AI 网关。
    """
    gw = get_gateway()
    feats = _clean(build_features(df, horizon))
    tr, te = _holdout(feats, test_size)
    cv = _purged_splits(tr)
    Xtr, Xte = tr[FEATURE_COLS], te[FEATURE_COLS]
    yc_tr, yc_te = tr[CLS_TARGET], te[CLS_TARGET]
    yr_tr, yr_te = tr[REG_TARGET], te[REG_TARGET]

    # 分类器:时序 CV 选超参(estimator 构造经网关)
    clf_p, clf_cv = _best_params(
        Xtr, yc_tr, lambda p: gw.make_estimator(p, "clf"), "roc_auc", cv)
    # 回归器:时序 CV 选超参
    reg_p, reg_cv = _best_params(
        Xtr, yr_tr, lambda p: gw.make_estimator(p, "reg"), "r2", cv)
    # 训练双模型(经网关)
    clf, reg = gw.fit_pair(Xtr, yc_tr, yr_tr, clf_p, reg_p)

    # 样本外评估(预测经网关)
    proba, pred_r = gw.predict_pair((clf, reg), Xte)
    pred_c = (proba > 0.5).astype(int)
    acc = accuracy_score(yc_te, pred_c)
    auc = (roc_auc_score(yc_te, proba)
           if yc_te.nunique() > 1 else float("nan"))
    r2 = r2_score(yr_te, pred_r)
    mae = mean_absolute_error(yr_te, pred_r)

    os.makedirs(MODELS_DIR, exist_ok=True)
    if model_path is None:
        model_path = os.path.join(MODELS_DIR, f"{symbol}.joblib")
    joblib.dump({"clf": clf, "reg": reg, "horizon": horizon, "label_protocol": "available-before-cutoff-v1",
                 "clf_params": clf_p, "reg_params": reg_p}, model_path)

    return {
        "model_path": model_path,
        "horizon": horizon,
        "label_protocol": "available-before-cutoff-v1",
        "purged_n": int(len(feats) - len(tr) - len(te)),
        "train_n": int(len(Xtr)), "test_n": int(len(Xte)),
        "clf_cv_auc": clf_cv, "reg_cv_r2": reg_cv,
        "test_accuracy": round(float(acc), 4),
        "test_auc": round(float(auc), 4) if not np.isnan(auc) else None,
        "test_r2": round(float(r2), 4),
        "test_mae": round(float(mae), 6),
        "clf_params": clf_p, "reg_params": reg_p,
        "feature_importances": dict(zip(
            FEATURE_COLS, [round(float(v), 4) for v in clf.feature_importances_])),
    }


def _train_pair_on(feats_slice: pd.DataFrame):
    """在给定切片上训练一对模型(经网关),返回 (clf, reg) 或 None。"""
    gw = get_gateway()
    sub = feats_slice.dropna(subset=FEATURE_COLS + [CLS_TARGET, REG_TARGET])
    if len(sub) < 50 or sub[CLS_TARGET].nunique() < 2:
        return None
    X, yc, yr = sub[FEATURE_COLS], sub[CLS_TARGET], sub[REG_TARGET]
    return gw.fit_pair(X, yc, yr, PARAM_GRID[0], PARAM_GRID[0])


def walk_forward_signals(df: pd.DataFrame, horizon: int = 5, lookback: int = 60,
                         window: int = 750, retrain_every: int = 20,
                         prob_threshold: float = 0.55,
                         return_threshold: float = 0.0) -> pd.Series:
    """逐日 walk-forward:过滤标签结束日期后训练,生成收盘后可用的 0/1 信号。

    门控:上涨概率 > prob_threshold 且 预测前向收益 > return_threshold 才持仓。
    第 t 日训练只含 label_end < date[t] 的记录,用第 t 日收盘特征预测。
    训练/预测均经 AI 网关本地后端。
    """
    gw = get_gateway()
    feats = build_features(df, horizon)
    n = len(df)
    position = pd.Series(0, index=df.index)
    if n <= lookback:
        return position

    clf, reg, trained_at = None, None, -9999
    for t in range(lookback, n):
        lo = max(0, t - window)
        if t - trained_at >= retrain_every or t == lookback:
            pair = _train_pair_on(_known_before(feats.iloc[lo:t], feats[FEATURE_DATE].iloc[t]))
            if pair is None:
                continue
            clf, reg = pair
            trained_at = t
        if clf is None:
            continue
        Xt = feats.iloc[[t]][FEATURE_COLS]
        if Xt.isna().any().any():
            continue
        prob_up, pred_ret = gw.score_pair((clf, reg), Xt)
        if prob_up > prob_threshold and pred_ret > return_threshold:
            position.iloc[t] = 1
    return position


# ———————————————— 跨股票联合(pooled)训练 ————————————————
def _pool_frame(dfs: list, horizon: int = 5) -> pd.DataFrame:
    """把多只股票特征拼接(带 symbol/date 标记),用于联合训练。"""
    frames = []
    for sym, d in dfs:
        f = build_features(d, horizon).copy()
        f["symbol"] = sym
        f["date"] = f[FEATURE_DATE]
        frames.append(f)
    return pd.concat(frames, ignore_index=True)


def train_pooled(dfs: list, model_path: str | None = None,
                 test_size: float = 0.15, horizon: int = 5) -> dict:
    """把全市场多只股票的特征拼接起来,训一个"大盘模型"(数据量最大化)。

    切分:按时间把最后 test_size 比例作为样本外测试(用 date 排序),避免用未来数据。
    """
    gw = get_gateway()
    pool = _pool_frame(dfs, horizon)
    pool = pool.dropna(subset=FEATURE_COLS + [CLS_TARGET, REG_TARGET])
    pool = pool.sort_values("date").reset_index(drop=True)
    tr, te = _holdout(pool, test_size)
    cv = _purged_splits(tr)

    clf_p, _ = _best_params(
        tr[FEATURE_COLS], tr[CLS_TARGET],
        lambda p: gw.make_estimator(p, "clf"), "roc_auc", cv)
    reg_p, _ = _best_params(
        tr[FEATURE_COLS], tr[REG_TARGET],
        lambda p: gw.make_estimator(p, "reg"), "r2", cv)
    clf, reg = gw.fit_pair(
        tr[FEATURE_COLS], tr[CLS_TARGET], tr[REG_TARGET], clf_p, reg_p)

    proba, pred_r = gw.predict_pair((clf, reg), te[FEATURE_COLS])
    pred_c = (proba > 0.5).astype(int)
    acc = accuracy_score(te[CLS_TARGET], pred_c)
    auc = (roc_auc_score(te[CLS_TARGET], proba)
           if te[CLS_TARGET].nunique() > 1 else float("nan"))
    r2 = r2_score(te[REG_TARGET], pred_r)

    os.makedirs(MODELS_DIR, exist_ok=True)
    if model_path is None:
        model_path = os.path.join(MODELS_DIR, "pooled.joblib")
    joblib.dump({"clf": clf, "reg": reg, "pooled": True, "horizon": horizon,
                 "label_protocol": "available-before-cutoff-v1"}, model_path)
    return {
        "model_path": model_path, "pooled": True,
        "horizon": horizon, "label_protocol": "available-before-cutoff-v1",
        "purged_n": int(len(pool) - len(tr) - len(te)),
        "train_n": int(len(tr)), "test_n": int(len(te)),
        "test_accuracy": round(float(acc), 4),
        "test_auc": round(float(auc), 4) if not np.isnan(auc) else None,
        "test_r2": round(float(r2), 4),
        "feature_importances": dict(zip(
            FEATURE_COLS, [round(float(v), 4) for v in clf.feature_importances_])),
    }


def walk_forward_pooled(df: pd.DataFrame, dfs: list, model_path: str,
                        horizon: int = 5, lookback: int = 60, window: int = 750,
                        retrain_every: int = 60, prob_threshold: float = 0.55,
                        return_threshold: float = 0.0,
                        max_train_rows: int = 8000) -> pd.Series:
    """跨股票预测:按各股票标签结束日过滤,再限制历史窗口和行数。

    model_path 保留调用兼容性;此路径每次重新训练,不加载预先拟合的模型。
    """
    gw = get_gateway()
    # 特征只算一次(此前放在循环里,是主要的性能瓶颈)
    pool_all = _pool_frame(dfs, horizon).sort_values("date").reset_index(drop=True)
    feats_df = build_features(df, horizon)
    target_dates = feats_df[FEATURE_DATE]
    n = len(df)
    position = pd.Series(0, index=df.index)
    if n <= lookback:
        return position

    model = None
    trained_at = -9999
    for t in range(lookback, n):
        if t - trained_at >= retrain_every or t == lookback or model is None:
            known = _known_before(pool_all, target_dates.iloc[t])
            cut = known[known[FEATURE_DATE] >= target_dates.iloc[max(0, t - window)]].tail(max_train_rows)
            pair = _train_pair_on(cut)
            if pair is None:
                continue
            model = pair
            trained_at = t
        clf, reg = model
        Xt = feats_df.iloc[[t]][FEATURE_COLS]
        if Xt.isna().any().any():
            continue
        prob_up, pred_ret = gw.score_pair((clf, reg), Xt)
        if prob_up > prob_threshold and pred_ret > return_threshold:
            position.iloc[t] = 1
    return position
